...
const inputTarefa = document.getElementById("input-tarefa") as HTMLInputElement;
const inputPonto = document.getElementById("input-ponto") as HTMLInputElement;
const btnAdicionar = document.getElementById("adicionar-tarefa") as HTMLButtonElement;
const listaAbertas = document.getElementById("lista-tarefas-abertas") as HTMLUListElement;
const listaConcluidas = document.getElementById("lista-tarefas-concluidas") as HTMLUListElement;
const totalXp = document.getElementById("xp") as HTMLElement;
const mensagemListaVazia = document.getElementById("mensagem-lista-vazia") as HTMLElement;

// Variável que armazena o total de XP
let xpTotal: number = 0;

// Atualiza a exibição da mensagem de lista vazia
function atualizarMensagemListaVazia(): void {
...
function atualizarXpTotal(): void {
  totalXp.textContent = xpTotal.toString(); // Define o valor atualizado
}

// Cria um novo item de lista
function criarItemLista(titulo: string, xp: number, lista: HTMLUListElement): void {
  ...
  if (lista === listaAbertas) {
    ...
    botaoEditar.addEventListener("click", () => {
      const novoTitulo = prompt(
        "Digite um novo nome para o inimigo:",
      ...
      if (novoTitulo) {
        ...
        const novoXp = prompt(
          "Digite o novo valor de XP:",
          xpTarefa.textContent || ""
        );
...
btnAdicionar.addEventListener("click", (e: Event) => {
  e.preventDefault();
